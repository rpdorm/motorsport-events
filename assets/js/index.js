...
for (i=0; i<Object.keys(series).length; i++) {
	...
	for (j=0; j<Object.keys(events).length; j++) {
		...
		if (timestamp >= now && timestamp <= upcoming) {
			...
			$(".eventlist").append("<div  id='" + timestamp + "' class='card sortme' style='width: 18rem;'><div class=' card-header'><img class='card-img-top' src='assets/png/" + series[i].symbol + ".png'><h5 class='card-title text-center'>" + name + "</h5><h6 class='card-subtitle mb-2 text-muted text-center'>" + circuit.name + "</h6></div></div>");
